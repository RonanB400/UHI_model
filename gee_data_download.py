...
def download_landsat_data(use_drive=True, output_dir='./data'):
    ...
    if use_drive:
        # Export Landsat LST to Google Drive
        export_lst = ee.batch.Export.image.toDrive({
            'image': lst.clip(lyon_geometry),
            'description': 'Lyon_LST',
            'folder': 'UHI_Lyon',
            'scale': 30,
        ...
        export_lst.start()
        print("LST export task started on Google Drive")
    else:
    ...
    if use_drive:
        for i, date in enumerate(dates):
            image = ee.Image(lst_collection.filter(ee.Filter.eq('date', date)).first())
            task = ee.batch.Export.image.toDrive({
                'image': image.clip(lyon_geometry),
                'description': f'Lyon_LST_{date}',
                'folder': 'UHI_Lyon_TimeSeries',
                'scale': 30,
                'region': lyon_geometry,
                'maxPixels': 1e9
            })
            task.start()
            print(f"Exporting LST for {date} to Google Drive")
    else:
...
def download_sentinel_data(use_drive=True, output_dir='./data'):
    ...
    if use_drive:
        # Export Sentinel-2 NDVI to Google Drive
        export_ndvi = ee.batch.Export.image.toDrive({
            'image': ndvi.clip(lyon_geometry),
            'description': 'Lyon_NDVI',
            'folder': 'UHI_Lyon',
        ...
        export_ndvi.start()
        print("NDVI export task started on Google Drive")
        
        # Export Sentinel-2 true color
        rgb_bands = sentinel_image.select(['B4', 'B3', 'B2'])
        export_rgb = ee.batch.Export.image.toDrive({
            'image': rgb_bands.clip(lyon_geometry),
            'description': 'Lyon_S2_RGB',
            'folder': 'UHI_Lyon',
            'scale': 10,
            'region': lyon_geometry,
            'maxPixels': 1e9
        ...
        export_rgb.start()
        print("RGB export task started on Google Drive")
    else:
        # Create output directory if it doesn't exist
        os.makedirs(output_dir, exist_ok=True)
...
if __name__ == "__main__":
    ...
    download_landsat_data(use_drive=use_google_drive, output_dir=output_directory)
    download_sentinel_data(use_drive=use_google_drive, output_dir=output_directory)
